...
from pipeline_lib.registry import load_registry, store_schema, find_existing_schema, generate_registry_key
from pipeline_lib.schema import fingerprint_columns, jaccard_similarity, generate_schema_with_ai
from pipeline_lib.loader import load_file, preview_file, get_supported_formats, detect_format
from pipeline_lib.cleaning import (
    apply_silver_layer,
...
from pipeline_lib.validation import (
    apply_outlier_detection,
    apply_data_type_validation,
    apply_pattern_validation,
    apply_referential_integrity
)
from pipeline_lib.advanced_cleaning import apply_all_advanced_cleaning
from pipeline_lib.normalization import apply_automatic_normalization, generate_normalization_summary
from pipeline_lib.reporting import generate_data_quality_report, print_data_quality_report
...
def process_dataset(file_path, domain=None, dataset_name=None, **loader_kwargs):
    ...
    # Advanced cleaning (text normalization, range validation, smart filling, etc)
    print("  → Applying advanced cleaning...")
    rows_before_advanced = df.height
    df, advanced_stats = apply_all_advanced_cleaning(df, schema["official_standard"])
    rows_after_advanced = df.height
    print(f"    ✓ Advanced cleaning complete ({rows_before_advanced} → {rows_after_advanced} rows)")
    ...
    # =========================================================================
    # STEP 6: AUTOMATIC STATISTICAL NORMALIZATION
    # =========================================================================
    print("\n📊 Step 6: Applying Statistical Normalization...")
    df, normalization_stats = apply_automatic_normalization(df)
    normalization_summary = generate_normalization_summary(normalization_stats)
    
    if normalization_summary:
        print("  Normalizations applied:")
        for summary_line in normalization_summary:
            print(f"    {summary_line}")
    else:
        print("  No normalizations needed")
    
    print(f"  ✅ {df.height:,} rows × {len(df.columns)} columns (after normalization)")
...
if __name__ == "__main__":
    ...
    from pipeline_lib.loader import get_supported_formats
